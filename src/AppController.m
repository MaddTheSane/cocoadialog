...
- (CDControl *) chooseControl:(NSString *)runMode useOptions:options addExtraOptionsTo:(NSMutableDictionary *)extraOptions
{
	if (runMode == nil || [runMode isEqualToString:@"--help"]) {
		...
		return [[CDFileSelectControl alloc] initWithOptions:options];
	} else if ([runMode isEqualToString:@"filesave"]) {
		return [[CDFileSaveControl alloc] initWithOptions:options];
	} else if ([runMode isEqualToString:@"msgbox"]) {
		return [[CDMsgboxControl alloc] initWithOptions:options];
	} else if ([runMode isEqualToString:@"yesno-msgbox"]) {
		return [[CDYesNoMsgboxControl alloc] initWithOptions:options];
	} else if ([runMode isEqualToString:@"ok-msgbox"]) {
		return [[CDOkMsgboxControl alloc] initWithOptions:options];
	} else if ([runMode isEqualToString:@"textbox"]) {
		return [[CDTextboxControl alloc] initWithOptions:options];
	} else if ([runMode isEqualToString:@"progressbar"]) {
		return [[CDProgressbarControl alloc] initWithOptions:options];
	} else if ([runMode isEqualToString:@"inputbox"]) {
		return [[CDInputboxControl alloc] initWithOptions:options];
	} else if ([runMode isEqualToString:@"standard-inputbox"]) {
		return [[CDStandardInputboxControl alloc] initWithOptions:options];
		...
		return [[CDPopUpButtonControl alloc] initWithOptions:options];
	} else if ([runMode isEqualToString:@"standard-dropdown"]) {
		return [[CDStandardPopUpButtonControl alloc] initWithOptions:options];
	} else if ([runMode isEqualToString:@"bubble"]) {
		return [[CDBubbleControl alloc] initWithOptions:options];
	} else {
		NSFileHandle *fh = [NSFileHandle fileHandleWithStandardOutput];
